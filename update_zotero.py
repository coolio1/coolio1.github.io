...
FIELD = {
    'title': 1, 'abstractNote': 2, 'date': 6, 'DOI': 8, 'url': 10,
    'volume': 22, 'publisher': 25, 'place': 26, 'ISBN': 28, 'pages': 35,
    'publicationTitle': 41, 'bookTitle': 53, 'issue': 67,
    'journalAbbreviation': 85, 'institution': 90, 'thesisType': 120,
    'university': 121, 'conferenceName': 68,
}

# Item type IDs
ITEM_TYPE = {
    'book': 7, 'bookSection': 8, 'conferencePaper': 11,
    'journalArticle': 22, 'report': 34, 'thesis': 37,
}
...
def set_field(cur, item_id, field_name, value):
    ...
    cur.execute(
        'SELECT * FROM itemData WHERE itemID = ? AND fieldID = ?',
        (item_id, field_id)
    )
    if cur.fetchone():
        cur.execute(
        ...
        print(f'  [UPDATE] {field_name} = {value[:80]}...' if len(value) > 80 else f'  [UPDATE] {field_name} = {value}')
    else:
        cur.execute(
            'INSERT INTO itemData (itemID, fieldID, valueID) VALUES (?, ?, ?)',
            (item_id, field_id, value_id)
        )
        print(f'  [ADD] {field_name} = {value[:80]}...' if len(value) > 80 else f'  [ADD] {field_name} = {value}')
...
def add_creator(cur, item_id, first_name, last_name, creator_type='author', order_index=None):
    ...
    if order_index is None:
        cur.execute(
            'SELECT MAX(orderIndex) FROM itemCreators WHERE itemID = ?',
            (item_id,)
        )
...
def main():
    ...
    set_field(cur, 92, 'conferenceName',
    ...
    set_field(cur, 93, 'title', 'Espaços verdes e rio Ul — Diagnóstico e Plano de Acção')
    set_field(cur, 93, 'date', '2004-05')
    set_field(cur, 93, 'place', 'São João da Madeira')
    set_field(cur, 93, 'institution',
    ...
    print(f'  Backup em: C:\\Users\\quent\\Zotero\\zotero.sqlite.backup_claude_*')
    conn.close()
